...
这是 AGE 动漫自动追更脚本的软件化桌面项目，当前版本为 `0.9.2`。核心入口是 `anime_updater.js`，桌面入口是 `electron/main.js`。
...
- 修改行为、依赖、路径、调度或用户流程时，同步 `VERSION`、`README.md`、`使用说明.md` 和必要的 `BLOCKED.md`。
- 运行记录写入 `PROGRESS.md` 属于用户现场数据，未经确认不删除或回滚。
- 先运行 `pnpm test`、`git diff --check` 和 `git status --short --ignored`，再提交或发布。

## 当前边界

当前已覆盖配置面板、番剧启停、dry-run/下载控制、aria2/ffmpeg/IDM 链路、进度与清单。多站点扩展、安装包和更大范围重构仍在 `BLOCKED.md` 中，除非用户明确要求，不要擅自扩展。
