...
> 状态：0.10.0实现完成；2026-09-01 已安装到真实mpv并启用，部署哈希匹配。自动测试与停用态Lua加载已验证；一部长视频的双90%改名端到端实机验收仍未记录。初版日期：2026-08-29；实现日期：2026-09-01；状态复核：2026-09-01。

## 目标与不可变语义

- 只处理 `D:\idm下载` 下、能够由当前动漫的 `folder_name` 模板唯一匹配的本地目录。
- “看完”必须同时满足：有效真实播放时间 `eligible_seconds >= duration × 0.9`，以及历史最大播放位置 `max_time_pos >= duration × 0.9`。
...
6. **达标**：Node按稳定身份汇总历史状态和所有未提交记录。只有 `eligible_seconds >= duration × 0.9` 且 `max_time_pos >= duration × 0.9` 才生成合格事件。生成后从有效时长扣除一个阈值并把最大位置重置为0，使同一文件再次达标时也必须重新通过两道门槛。
...
以下视频时长均为20分钟，门槛均为18分钟：

1. 1倍速有效播放18分钟，最大位置达到18分钟：两项都满足，生成达标事件。
2. 2倍速从头完整看完，现实约经过10分钟：位置满足、时长不足，不达标。
3. seek到19分钟，只实际播放2分钟：位置满足、时长不足，不达标。
4. 反复播放结尾片段，累计有效时间达到18分钟且最大位置超过18分钟：会达标。这是双90%仍无法排除的已知限制，若要证明看过不同画面必须记录区间覆盖率，本轮不引入。
...
同一动漫第8集先同时满足两道90%，随后第3集也同时满足。两条合格事件顺序为8、3；关闭后每个目录只取最后一条，因此无论原前缀是多少，目标前缀为 `3_`。这不是取最大值。
...
| 2倍速 | 按真实经过秒数累计，不乘2；完整看一遍通常仍因时长不足而不达标 |
...
2. **双90%可由固定状态表达。** 官方提供 `time-pos`、`duration`、`mp.get_time()`、周期计时器，以及暂停、核心空闲、缓冲、寻址属性。复现：`mpv --list-properties` 搜索这些名称。
